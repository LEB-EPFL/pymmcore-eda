...
def test_frap_map_handling():
    ...
    if not np.array_equal(event_map, expected_map):
        ...
        if np.array_equal(event_map, map2):
            print("It appears the second map is replacing first map instead extending.")
        elif np.array_equal(event_map, map1):
            print(
                "It appears the first map is being kept and the second map is ignored."
            )

    # Verify the map has been logically OR'd
    np.testing.assert_array_equal(
        event_map, expected_map, "FRAP map was not correctly combined with logical OR"
    )
